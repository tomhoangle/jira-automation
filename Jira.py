...
#access type field = 110002
#confluence link field = 110001
...
def show_template(inName):
    ...
    if ("AD(DAI)" in accessList):
        daiButton.select()
    if("AD(ODS)" in accessList):
        odsButton.select()
    if("Jira" in accessList):
        jiraButton.select()
    if("Email" in accessList):
        emailButton.select()
    if("Slack" in accessList):
        slackButton.select()
    if("Phone" in accessList):
        phoneButton.select()
